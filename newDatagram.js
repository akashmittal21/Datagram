...
function newDatagram() {
	...
	createNode.addEventListener("click", function() {
		...
		const tableData = tableRows.map(row => [row.querySelector("td:first-child").textContent, row.querySelector("input").value]);
		...
		const attributes = tableRows.map(row => `${row.querySelector("td:first-child").textContent}:${row.querySelector("input").value}`).join(",");
      ...
      `;
          ...
          `).join("");
				...
				createNodeBtn.addEventListener("click", () => {
					...
					const categorySelect = popoverContent.querySelector("#categorySelect").value;
					const tableRows = Array.from(popoverContent.querySelectorAll("tbody tr"));
					...
					const generateTestCasesBtn = document.querySelector("#generate-test-btn");
